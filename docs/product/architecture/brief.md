...
| `computeFitViewport`                       | `packages/renderer/src/geometry.ts`      | `(nodes: readonly {x,y,radius}[], annotations: readonly {x,y,width,height}[], canvasWidth: number, canvasHeight: number) => Viewport`                                                                                   | pure-function (return-only); guards zero-width/height bounding box (0/1-node cases) with a minimum-extent substitution; clamps zoom to `[RESET_VIEW_ZOOM_FLOOR, ZOOM_MAX]` — decoupled from `clampZoom`'s manual-zoom floor per ADR-005 |
