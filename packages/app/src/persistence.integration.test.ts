...
describe('persistence round-trip (integration)', () => {
  ...
  it('shareGraph → loadFromUrl restores the graph exactly', async () => {
    ...
    const sharedUrl = (navigator.clipboard.writeText as ReturnType<typeof vi.fn>).mock.calls[0][0]
    ...
    expect(restored.nodes.map((n) => n.id)).toEqual(withEdge.nodes.map((n) => n.id))
    expect(restored.nodes.map((n) => [n.x, n.y])).toEqual(withEdge.nodes.map((n) => [n.x, n.y]))
