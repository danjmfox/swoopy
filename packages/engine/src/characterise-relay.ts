...
function relayInject(state: RState, edges: REdge[], nodeId: string, strength: number, MAX_HOPS: number): void {
...
function emitFragments(signals: RSignal[], edge: REdge, strength: number, hopsRemaining: number): void {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const preAdvance = staggerTicks > 0 ? (i / count) * (SIGNAL_SPEED * (1 / 60) * EDGE_TRANSIT_TICKS) : 0
    signals.push({
      edgeId: edge.id,
...
function relayStep(state: RState, edges: REdge[], dt: number, MAX_HOPS: number): void {
  ...
  for (const s of state.signals) {
    ...
    if (advanced.progress >= 1) {
      ...
      node.value = Math.min(node.max, Math.max(node.min, node.value + s.strength * edge.polarity))
...
function runScenario(state: RState, edges: REdge[], MAX_HOPS: number, nodeIds: string[]): ScenarioResult {
  ...
  const checkAt = new Set([EDGE_TRANSIT_TICKS, Math.round(runTicks / 2), runTicks])
  let peakSignals = 0
  ...
  for (let t = 0; t < runTicks; t++) {
    ...
    if (checkAt.has(t + 1)) {
      snapshots[t + 1] = Object.fromEntries(
        nodeIds.map(id => [id, Math.round((state.nodes.get(id)?.value ?? 0) * 100) / 100])
      )
    }
  }
  ...
  return {
    snapshots,
    peakSignals,
    finalValues: Object.fromEntries(nodeIds.map(id => [id, state.nodes.get(id)?.value ?? 0])),
  }
}

function verdict(name: string, result: ScenarioResult, nodeIds: string[]): string {
  const final = result.finalValues
  if (name === 'reinforcing') {
    const saturated = nodeIds.every(id => final[id] >= 9.9)
    return saturated ? 'SATURATE ✓' : `PARTIAL  (A=${final.A?.toFixed(2)} B=${final.B?.toFixed(2)})`
  }
  if (name === 'balancing') {
...
const scenarios: Array<{ name: string; label: string; fn: (hops: number) => ScenarioResult; nodes: string[] }> = [
  { name: 'reinforcing', label: 'Reinforcing loop  A→B→A (+/+)', fn: scenarioReinforcing, nodes: ['A', 'B'] },
  { name: 'balancing',   label: 'Balancing loop    A→B→A (+/-)', fn: scenarioBalancing,   nodes: ['A', 'B'] },
  { name: 'diamond',     label: 'Diamond           A→B→D(+)/A→C→D(-)', fn: scenarioDiamond, nodes: ['A', 'B', 'C', 'D'] },
]

console.log('=== Relay propagation characterisation ===')
console.log(`SIGNAL_SPEED=${SIGNAL_SPEED}  EDGE_TRANSIT_TICKS=${EDGE_TRANSIT_TICKS}`)
console.log()

for (const scenario of scenarios) {
  console.log(`--- ${scenario.label} ---`)
  console.log(`${'MAX_HOPS'.padEnd(10)} ${'peak-sig'.padEnd(10)} final vals                      verdict`)
  for (const hops of MAX_HOPS_CANDIDATES) {
    const result = scenario.fn(hops)
    const finalStr = scenario.nodes.map(id => `${id}=${result.finalValues[id]?.toFixed(2)}`).join(' ')
...
console.log('Recommendation: choose lowest MAX_HOPS where all three verdicts pass.')
console.log('Record choice and evidence in DR--20260401--engine--relay-propagation-model.')
